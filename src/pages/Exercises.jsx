...
function ExerciseForm({ exercise, onSubmit, busy }) {
  return <form onSubmit={onSubmit} className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="sm:col-span-2 rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Valores sugeridos al agregarlo a una rutina</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><label className="text-sm font-bold text-slate-600">Series<input name="default_sets" type="number" min="1" max="20" defaultValue={exercise?.default_sets || 3} className={input} /></label><label className="text-sm font-bold text-slate-600">Repeticiones<input name="default_reps" defaultValue={exercise?.default_reps || "8-12"} className={input} /></label><label className="text-sm font-bold text-slate-600">Descanso seg.<input name="rest_seconds" type="number" min="0" max="1800" step="5" defaultValue={exercise?.rest_seconds ?? 60} className={input} /></label></div></div>
...
function AliasList({ aliases = [] }) {
  ...
  const shown = aliases.slice(0, 8);
  return <div className="mt-3 rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Alias de búsqueda</p><div className="mt-2 flex flex-wrap gap-1.5">{shown.map((alias) => <span key={alias} className="rounded-full bg-white px-2.5 py-1 text-[10px] font-bold leading-4 text-slate-600 shadow-sm">{alias}</span>)}{aliases.length > shown.length && <span className="rounded-full bg-slate-200 px-2.5 py-1 text-[10px] font-black text-slate-500">+{aliases.length - shown.length}</span>}</div></div>;
}

function VariantLabel({ exercise }) {
  if (!exercise?.has_variants) return null;
  return <span className="mt-1 inline-flex rounded-full bg-amber-50 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-amber-700">Variante · ID {exercise.library_code}</span>;
}
...
export default function Exercises() {
  ...
  return <div className="mx-auto max-w-[1480px] space-y-4 sm:space-y-6">
    ...
    <section className="grid gap-3 md:grid-cols-2">{shown.map((exercise) => { const open = openId === exercise.id; return <article key={exercise.id} className="overflow-hidden rounded-[22px] border border-black/8 bg-white shadow-sm">
      <button onClick={() => setOpenId(open ? "" : exercise.id)} className="flex min-h-16 w-full items-start justify-between gap-4 p-4 text-left"><div className="flex min-w-0 flex-1 items-start gap-3"><span className="mt-0.5 grid size-11 shrink-0 place-items-center rounded-2xl bg-red-50 text-[#E30613]"><Dumbbell className="size-5" /></span><div className="min-w-0 flex-1"><p className="break-words font-black leading-5 text-slate-900">{exercise.name}</p><VariantLabel exercise={exercise} /><p className="mt-1 break-words text-xs font-bold leading-4 text-slate-400">{exercise.muscle_group}{exercise.equipment ? ` · ${exercise.equipment}` : ""}</p></div></div>{open ? <ChevronUp className="mt-1 size-4 shrink-0 text-slate-400" /> : <ChevronDown className="mt-1 size-4 shrink-0 text-slate-400" />}</button>
      {open && <div className="border-t border-slate-100 p-4"><h2 className="text-lg font-black leading-6 text-slate-900">{exercise.name}</h2><VariantLabel exercise={exercise} /><p className="mt-2 text-sm leading-6 text-slate-600">{exercise.notes || "La explicación breve de este ejercicio se completará más adelante."}</p><AliasList aliases={exercise.aliases} /><div className="mt-3 rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Referencia</p><p className="mt-1 text-xs font-bold text-slate-600">{exercise.category} · {exercise.default_sets || 3} series · {exercise.default_reps || "8-12"} reps · {exercise.rest_seconds ?? 60}s</p></div><div className="mt-4 overflow-hidden rounded-2xl bg-slate-100"><ExerciseGif exercise={exercise} /></div><div className="mt-3 flex flex-wrap gap-2">{exercise.video_url && <a href={exercise.video_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-700"><Video className="size-3.5" /> Ver video <ExternalLink className="size-3" /></a>}{canEdit(exercise) && <button onClick={() => setEditing(exercise)} className="inline-flex items-center gap-1.5 rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-700"><Pencil className="size-3.5" /> Editar</button>}{canEdit(exercise) && !exercise.is_system && <button onClick={() => remove(exercise)} disabled={busy} className="inline-flex items-center gap-1.5 rounded-xl bg-red-50 px-3 py-2 text-xs font-black text-[#9E0710]"><Trash2 className="size-3.5" /> Eliminar</button>}</div></div>}
