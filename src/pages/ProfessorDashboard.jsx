...
export default function ProfessorDashboard() {
  ...
  return <div className="mx-auto max-w-[1480px] space-y-6">
    <section className="page-head"><div><p className="eyebrow">Panel profesor</p><h1 className="page-title">Hola, {profile?.display_name || "Profe"}</h1><p className="page-subtitle">Vista operativa sin acceso a caja, reportes financieros ni administración.</p></div></section>
    ...
    <section className="grid gap-6 xl:grid-cols-[1.45fr_.85fr]">
      <div className="panel"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="section-title">Alumnos</h2><p className="mt-1 text-xs text-slate-500">Consulta rápida. Los datos administrativos sensibles permanecen ocultos.</p></div><label className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 px-3"><Search className="size-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="bg-transparent text-sm outline-none" placeholder="Nombre o DNI" /></label></div>
